...
ensure_local_host_flake() {
    local username home_directory
    username=$(printf '%s' "$USER" | jq -Rs .)
    home_directory=$(printf '%s' "$HOME" | jq -Rs .)
    ...
    cat > nix/hosts/local/flake.nix <<EOF
{
  ...
  outputs = { ... }: {
    homeModules.default = { lib, ... }: {
      ...
      home.homeDirectory = lib.mkForce $home_directory;
    };
  };
}
EOF
