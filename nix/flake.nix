  ...
  outputs = { self, nixpkgs, home-manager, ... }@inputs:
    let
      ...
      mkHomeConfiguration = system: username:
        let
          # Keep the real home dir (including symlinks) by preferring $HOME when present.
          # Falls back to the conventional location if $HOME is unset during evaluation.
          homeDirectory = let
            envHome = builtins.getEnv "HOME";
            defaultHome = if nixpkgs.lib.hasSuffix "-darwin" system
              then "/Users/${username}"
              else "/home/${username}";
          in nixpkgs.lib.mkDefault (if envHome != "" then envHome else defaultHome);
        in
