...
if ioreg -r -k AppleClamshellState | grep -q '"AppleClamshellState" = Yes'; then
  # 1. Trigger the screensaver
  open -b com.apple.ScreenSaver.Engine

  # 2. Give macOS a brief moment to process the state change
  sleep 2

  # 3. Query the system registry to check the lock status
